...
/*   Updated: 2024/02/17 20:36:26 by mjochum          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_drawplayer(t_player *player, t_vars *vars)
{
	...
	(void) vars;
	start = (t_pixel){(player->xpos * CELL_SIZE) - 2, (player->ypos * CELL_SIZE) - 2, 0xff0000};
	dest = (t_pixel){(player->xpos * CELL_SIZE) + 2, (player->ypos * CELL_SIZE) + 2, 0xff0000};
	while (start.y <= dest.y)
	{
		ft_drawline(start, (t_pixel){dest.x, start.y, start.colour}, vars->buffer, vars);
		start.y++;
	}
	start = (t_pixel){player->xpos * CELL_SIZE, player->ypos * CELL_SIZE, 0xff0000};
